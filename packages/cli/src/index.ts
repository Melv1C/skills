...
async function authSource(): Promise<AuthSource> {
  ...
  throw new Error("Not authenticated. Run `agent-tools auth login` or set SKILLS_API_TOKEN.");
}
  ...
  if (storedToken && !force) {
    try {
      ...
      if (error instanceof ApiError && error.status === 401) {
        throw new Error("Stored token is invalid. Run `agent-tools auth login --force`.");
      }
      throw error;
    }
  }
...
try {
  ...
  process.stderr.write(`agent-tools: ${message}\n`);
